...
namespace Game.World
{
    ...
    public class World
    {
        ...
        public event EventHandler<ISystem> OnSystemRegist = (sender, system) => { Console.WriteLine(system.GetType().Name + " regist"); };
        public event EventHandler<ISystem> OnSystemUnRegist = (sender, system) => { Console.WriteLine(system.GetType().Name + " unregist"); };
        ...
        public void RegistSystem<T>() 
            where T : ISystem , new()
        {
            if (IsContainSystem<T>()) {return; }
            T t = new T();
            _systems.Add(t);
        ...
        public void UnRegistSystem<T>() where T : ISystem {
            if (!IsContainSystem<T>()) { return; }
            T t = GetSystem<T>();
            _systems.Remove(t);
